...
export function moderateAdminMarketplace(input) {
  return mutateAdminMarketplace(normalizeMarketplaceModeration(input));
}
...
export function setAdminUserRole(input) {
  return mutateAdminUserRole(normalizeUserRoleChange(input));
}

export function setAdminUserStatus(input) {
  return mutateAdminUserStatus(normalizeUserStatusChange(input));
}
...
export async function getAdminReports(request) {
  ...
  return {
    ...
    items: result.items.map((item) => ({
      ...item,
      reporter_tour_reports_24h: Number(item.reporter_tour_reports_24h ?? 0),
    })),
  };
}

export function reviewAdminReport(input) {
  return mutateAdminReport(normalizeReportDecision(input));
}
...
export function resolveAdminSupportRequest(input) {
  return mutateAdminSupportRequest(normalizeSupportResolution(input));
}
...
export function addAdminCategory(input) {
  return insertAdminCategory(normalizeCategoryCreate(input));
}

export function updateAdminCategory(input) {
  return mutateAdminCategory(normalizeCategoryUpdate(input));
}
...
export async function getAdminOperationalHealth(hours = 24) {
  ...
  const [data, notificationFanout] = await Promise.all([
    fetchAdminOperationalHealth(windowHours),
    fetchAdminNotificationFanoutHealth(),
  ]);
  ...
  return {
    ...
    openAlerts: Array.isArray(data?.open_alerts) ? data.open_alerts.map((alert) => ({
      ...alert,
      observed: Number(alert?.observed ?? 0),
      threshold: Number(alert?.threshold ?? 0),
    })) : [],
    metrics: Object.fromEntries(Object.entries(metrics).map(([name, metric]) => [name, {
      samples: Number(metric?.samples ?? 0),
      average: Number(metric?.average ?? 0),
      maximum: Number(metric?.maximum ?? 0),
    }])),
    queues: {
      ...numericRecord(data?.queues),
      notification_fanout_pending: Number(notificationFanout?.pending ?? 0),
      notification_fanout_claimed: Number(notificationFanout?.claimed ?? 0),
      notification_fanout_dead_lettered: Number(notificationFanout?.dead_lettered ?? 0),
      notification_fanout_oldest_pending_seconds: Number(notificationFanout?.oldest_pending_seconds ?? 0),
      notification_fanout_stale_claims: Number(notificationFanout?.stale_claims ?? 0),
    },
