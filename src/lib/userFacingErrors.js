const TECHNICAL_PATTERN = /(?:postgres|postgrest|supabase|sqlstate|row-level security|rls|jwt|fetch failed|failed to fetch|networkerror|typeerror|referenceerror|syntaxerror|constraint|duplicate key|violates|rpc|edge function|status code|\b[45]\d\d\b|[a-z_]{3,}\([^)]*\)|\bPGRST\d+\b|\b23\d{3}\b)/i;
...
const RULES = [
  ...
  { pattern: /blocked/i, message: 'This action is unavailable because the conversation is blocked.' },
  ...
  { pattern: /mime|file type|mp4|mov|webm|unsupported.*video/i, message: 'Choose an MP4, MOV, or WebM video.' },
...
export function userFacingError(error, fallback = 'Something went wrong. Please try again.') {
  ...
  return message.replace(/\s+/g, ' ').replace(/[.;:]?\s*(?:code|details|hint):.*$/i, '').trim() || fallback;
}
