  ...
  return (
    <div className="min-h-[100dvh] bg-background p-4 sm:p-6">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        <AdminMobileList label="Peek moderation records">
          {queue.isLoading ? <Card className="p-4"><ListRowsSkeleton rows={5} label="Loading Peek queue" /></Card> : data.items.length === 0 ? <Card className="p-6 text-center text-sm text-muted-foreground">No Peeks match these filters.</Card> : data.items.map((item) => <AdminMobileRecord key={item.tour_id} heading={<Link to={item.parent_path || '#'} target="_blank" rel="noopener noreferrer" className="inline-flex max-w-full items-center gap-1 hover:text-primary hover:underline"><span className="line-clamp-2">{item.parent_title || 'Untitled listing'}</span><ExternalLink className="h-3.5 w-3.5 shrink-0" /></Link>} summary={`${item.parent_type} · ${String(item.tour_id).slice(0, 8)} · ${Number(item.duration_seconds || 0).toFixed(1)} seconds`} badges={<><Badge variant="secondary">{item.status}</Badge><Badge variant={item.moderation_status === 'approved' ? 'default' : item.moderation_status === 'rejected' ? 'destructive' : 'outline'}>{item.moderation_status}</Badge></>} fields={[{ label: 'Owner', value: item.owner_name || 'Unknown', wide: true }, { label: 'Owner email', value: maskEmail(item.owner_email), wide: true }, { label: 'Reports', value: Number(item.report_count || 0) }, { label: 'Created', value: new Date(item.created_at).toLocaleString(), wide: true }, { label: 'Failure', value: item.failure_message, wide: true }, { label: 'Previous reason', value: item.rejection_reason, wide: true }]} actions={canReview(item) ? <><Button className="min-h-11 flex-1" size="sm" onClick={() => { setDecision({ item, action: 'approve' }); setReason(''); setConfirmation(''); }}><CheckCircle2 className="mr-1 h-4 w-4" />Approve</Button><Button className="min-h-11 flex-1" size="sm" variant="destructive" onClick={() => { setDecision({ item, action: 'reject' }); setReason(''); setConfirmation(''); }}><ShieldX className="mr-1 h-4 w-4" />Reject</Button></> : <span className="text-xs text-muted-foreground">No decision available</span>} />)}
        </AdminMobileList>
        <Card className="hidden overflow-hidden md:block"><div className="overflow-x-auto"><table className="w-full min-w-[1040px] text-sm"><thead><tr className="border-b bg-muted/30 text-left"><th className="p-3">Peek</th><th className="p-3">Owner</th><th className="p-3">Processing</th><th className="p-3">Reports</th><th className="p-3">Created</th><th className="p-3">Actions</th></tr></thead><tbody>
