...
export function fetchAdminAuditLog(request) { return rpc('admin_unified_audit_activity_page', { p_query: request.query, p_target_type: request.targetType, p_limit: request.limit, p_cursor_at: request.cursor?.at ?? null, p_cursor_id: request.cursor?.id ?? null }, 'We could not load the audit log.'); }
...
export function fetchAdminRecommendationConfiguration() { return rpc('admin_recommendation_configuration_snapshot', {}, 'We could not load recommendation configuration.'); }
export function purgeAdminRecommendationCache(serviceName, subjectListingId = null) { return rpc('admin_purge_recommendation_service_cache_v1', { p_service_name: serviceName, p_subject_listing_id: subjectListingId, p_limit: 5000 }, 'The recommendation cache could not be purged.'); }
