      ...
      {selected && (
        <div className="locked-bottom-sheet absolute inset-x-0 bottom-[72px] z-30 p-3.5">
          ...
          <button type="button" onClick={() => navigate(path(selected))} className="grid w-full grid-cols-[112px_minmax(0,1fr)] gap-3 pr-8 text-left">
            <div className="h-28 overflow-hidden rounded-xl border border-border bg-muted">{image(selected) ? <img src={image(selected)} alt="" className="h-full w-full object-cover" /> : <span className="flex h-full items-center justify-center"><ImageOff className="h-6 w-6 text-muted-foreground" /></span>}</div>
