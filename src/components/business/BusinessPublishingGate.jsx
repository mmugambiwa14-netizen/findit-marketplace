...
function BusinessApplication({ access, onBack, onSubmitted }) {
  ...
  if (access?.applicationStatus === 'needs_information') {
    const submitResponse = async (event) => {
      ...
      if (reportInvalidForm(event.currentTarget, 'Complete the required information before sending.')) return;
      setSubmitting(true);
      try {
  ...
  const submit = async (event) => {
    event.preventDefault();
    if (reportInvalidForm(event.currentTarget, 'Complete the highlighted required fields before submitting.')) return;
