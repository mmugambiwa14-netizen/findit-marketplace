...
export default function TourFailureState({ tour, onRetry = null, retrying = false, onRemove = null, removing = false }) {
  ...
  const message = rejected
    ? tour?.rejection_reason || 'This Peek is unavailable. Upload a corrected video below.'
    ...
    <div className="rounded-xl border border-destructive/25 bg-destructive/5 p-4" role="alert">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold">{rejected ? 'Peek unavailable' : 'Peek processing failed'}</p>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {onRemove && <Button type="button" size="sm" variant="ghost" onClick={onRemove} disabled={retrying || removing}><Trash2 />{removing ? 'Removing...' : rejected ? 'Remove unavailable Peek' : 'Discard this version'}</Button>}
          </div>
        </div>
      </div>
    </div>
