      ...
      <div className="mx-auto max-w-3xl px-4 py-4">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-[1fr_0.7fr]">
          <label htmlFor="tour-catalogue-search" className="relative block">
            ...
            />
            {query && (
              <button type="button" aria-label="Clear Peek search" onClick={() => onQueryChange('')} className="absolute right-1 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-lg text-muted-foreground hover:text-foreground">
                <X className="h-4 w-4" />
              </button>
            )}
          </label>
