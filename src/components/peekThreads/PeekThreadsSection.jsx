  ...
  return (
    <section id="peek-threads" className="surface-panel scroll-mt-24 overflow-hidden" aria-labelledby={`peek-threads-${parentId}`}>
      <div className="border-b border-border p-5 sm:p-6">
        ...
        {composerOpen && !isOwner && (
          <div className="mt-5 rounded-2xl border border-border bg-surface-secondary p-4">
            ...
            <select id={`peek-category-${parentId}`} value={category} onChange={(event) => setCategory(event.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-border bg-card px-3 text-sm">
            ...
            <div className="mt-2 flex flex-col gap-2 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap gap-2 sm:justify-end">
                {suggestedRequests(listingKind).slice(0, 3).map((suggestion) => (
                  <button key={suggestion.body} type="button" className="rounded-full border border-border px-2.5 py-1 hover:border-primary" onClick={() => { setCategory(suggestion.category); setBody(suggestion.body); }}>{suggestion.body}</button>
                ))}
              </div>
            </div>
      ...
      <AlertDialog open={Boolean(declineTarget)} onOpenChange={(open) => { if (!open && !declineMutation.isPending) { setDeclineTarget(null); setDeclineReason(''); } }}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Decline this Peek Request?</AlertDialogTitle><AlertDialogDescription>Give buyers a short, useful reason. The request will no longer appear publicly as awaiting your response.</AlertDialogDescription></AlertDialogHeader>
          {declineTarget?.body && <p className="rounded-xl border border-border bg-muted/30 p-3 text-sm font-medium">“{declineTarget.body}”</p>}
