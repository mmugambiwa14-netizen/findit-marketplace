-- Roll back migration 0092 by removing the public SECURITY INVOKER wrapper
...
begin
  ...
  if public_wrapper_count <> 1 then
    raise exception '0092 rollback expected one public marketplace view wrapper, found %', public_wrapper_count;
  end if;
  ...
  if private_implementation_count <> 1 then
    raise exception '0092 rollback expected one private marketplace view implementation, found %', private_implementation_count;
  end if;
  ...
  if restored_count <> 1 then
    raise exception '0092 rollback did not restore the canonical public marketplace view implementation';
  end if;
  ...
  if exists (
    ...
    raise exception '0092 rollback left the marketplace view implementation in private';
  end if;
end
$rollback$;
