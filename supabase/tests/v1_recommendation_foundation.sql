...
select extensions.has_table('public', 'recommendation_events', 'event table exists');
...
select extensions.is(
  ...
  7::bigint,
  'every planned recommendation service has one active weight profile'
);
select extensions.is(
  (select count(*)::bigint
...
select extensions.isnt(
  ...
  null::uuid,
  'guest records one bounded public view'
);
...
select extensions.isnt(
  ...
  null::uuid,
  'search event stores counts but not search text'
);
select extensions.throws_ok(
  $$select public.record_recommendation_event(
    p_event_type => 'view',
...
select extensions.is(public.is_admin(), false, 'moderator-labelled user has no admin authority');
select extensions.throws_ok(
...
select extensions.isnt(
  ...
  null::uuid,
  'admin creates an audited relationship'
);
select extensions.isnt(
...
select extensions.is(
  ...
  3::bigint,
  'each admin configuration change creates history'
);
select extensions.throws_matching(
...
select extensions.lives_ok(
  ...
  'service worker refreshes projections in a bounded batch'
);
select extensions.lives_ok(
...
select extensions.lives_ok(
  ...
  'service worker provisions a future event partition'
);
select extensions.ok(
  public.recommendation_foundation_health()
    ?& array['projection_version', 'projected_public_listings', 'active_taxonomy_nodes', 'active_relationships', 'active_weight_profiles', 'generated_at'],
  'service worker reads a payload-free health snapshot'
);
select extensions.throws_ok(
  $$update public.recommendation_configuration_audit set action = 'update'$$,
  '42501', 'recommendation configuration audit history is immutable',
  'trusted roles still cannot rewrite audit history'
);
