-- 0056_recommendation_projection_queue.sql
...
begin
  ...
  exception when others then
    -- Projection is advisory. Never abort or delay a listing write.
    null;
  end;
...
begin
  ...
  exception when others then
    -- Detail writes remain authoritative even when projection infrastructure fails.
    null;
  end;
...
begin
  if p_limit not between 1 and 500 then
    raise exception 'projection worker batch size must be between 1 and 500'
      using errcode = '22023';
  end if;
  if p_max_attempts not between 1 and 20 then
    raise exception 'projection worker max attempts must be between 1 and 20'
      using errcode = '22023';
  end if;
...
begin
  ...
  if not public.enqueue_listing_recommendation_projection(dead_letter.listing_id) then
    return false;
  end if;
...
values (
  ...
  true,
  'phase_1_implemented',
  '{"schema_version":56,"projection_version":1,"projection_mode":"asynchronous","services_enabled":false}'::jsonb
)
on conflict (control_key) do update set
  enabled = excluded.enabled,
  state = excluded.state,
