-- 0085_rls_auth_initplan_optimization.sql
--
-- Supabase's auth_rls_initplan advisor reports policies that call auth.uid()
-- directly in row predicates. PostgreSQL may then re-evaluate that function for
-- every candidate row. Wrapping the call in a scalar SELECT allows the planner
-- to calculate it once per statement without changing authorization semantics.
--
-- This migration is deliberately fail-closed. It fingerprints every affected
-- policy before altering it and preserves policy names, commands, roles and all
-- predicates other than the auth.uid() evaluation strategy.
