-- 0055_recommendation_partition_and_configuration_integrity.sql
...
begin
  ...
  -- Stop inserts from racing into the default partition while its rows for this
  -- month are moved into the newly attached range partition.
...
set
  ...
  configuration = configuration || '{"schema_version":55,"default_partition_migration":true,"audited_weight_activation":true,"bounded_health":true}'::jsonb,
  updated_at = now()
