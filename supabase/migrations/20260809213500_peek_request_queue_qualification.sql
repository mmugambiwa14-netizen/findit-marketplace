...
create or replace function private.seller_peek_request_queue(
...
returns table(
  request_id uuid, listing_id uuid, service_id uuid, parent_type text,
  parent_kind text, parent_title text, category peek_request_category, body text,
  supporter_count integer, created_at timestamptz, pending_seconds bigint,
  queue_score bigint, fulfilment_status text, fulfilment_attempt_count integer,
  fulfilment_tour_id uuid, fulfilment_expires_at timestamptz,
  fulfilment_failure_reason text, next_score bigint, next_created_at timestamptz,
  next_id uuid
)
language plpgsql
stable
...
begin
  ...
  page as (
    ...
    order by e.queue_score desc, e.created_at asc, e.id asc
    limit v_limit + 1
  ),
  visible as (
    select p.* from page p
    order by p.queue_score desc, p.created_at asc, p.id asc
    limit v_limit
  ),
  cursor_row as (
    select p.queue_score, p.created_at, p.id from page p
    order by p.queue_score desc, p.created_at asc, p.id asc
