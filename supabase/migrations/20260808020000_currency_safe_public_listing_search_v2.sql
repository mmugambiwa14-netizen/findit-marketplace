-- 20260808020000_currency_safe_public_listing_search_v2.sql
--
-- Public marketplace search v2.
--
-- The legacy search function predates seller-native multi-currency pricing and
-- the public/private location split. V2 keeps bounded keyset pagination while:
-- - treating native_price/native_currency as the price authority;
-- - allowing all currencies for newest/most-viewed browsing;
-- - requiring one explicit currency before price range filtering or price sort;
-- - returning public coordinates/location labels only;
-- - never returning raw seller contact values;
-- - excluding suspended content explicitly inside the SECURITY DEFINER body.

create index if not exists idx_listings_public_currency_price_asc
  on public.listings(country_code, kind, native_currency, native_price asc, id asc)
  where status in ('available', 'under_offer') and content_suspended_at is null;

create index if not exists idx_listings_public_currency_price_desc
  on public.listings(country_code, kind, native_currency, native_price desc, id desc)
  where status in ('available', 'under_offer') and content_suspended_at is null;

create or replace function private.public_listing_search_page_v2(
  p_kind text,
...
begin
  ...
  then
    raise exception 'invalid public listing search v2 page' using errcode = '22023';
  end if;
  ...
  if not exists (
    ...
    raise exception 'invalid public listing search v2 market' using errcode = '22023';
  end if;
  ...
  if normalized_currency <> '' and not exists (
    ...
    raise exception 'invalid public listing search v2 currency' using errcode = '22023';
  end if;
  ...
  if p_cursor_value is not null then
    ...
    exception when others then
      raise exception 'invalid public listing search v2 cursor' using errcode = '22023';
    end;
  end if;
  ...
  select
    ...
    listing.status::text,
    listing.created_via::text,
    listing.views,
    listing.created_at,
    listing.updated_at,
    case
      when listing.public_location_label is null then null
      else jsonb_build_object(
        'id', null,
        'name', listing.public_location_label,
        'type', 'public'
      )
    end,
...
grant execute on function private.public_listing_search_page_v2(
  text, text, text, text, text, text, numeric, numeric, integer, text, text,
  text, text, text, text, uuid, integer
) to anon, authenticated, service_role;

create or replace function public.public_listing_search_page_v2(
  p_kind text,
  p_country_code text default 'ZW',
  p_currency text default '',
  p_query text default '',
...
as $$
  select *
  from private.public_listing_search_page_v2(
    p_kind,
    p_country_code,
    p_currency,
    p_query,
...
grant execute on function public.public_listing_search_page_v2(
  text, text, text, text, text, text, numeric, numeric, integer, text, text,
  text, text, text, text, uuid, integer
) to anon, authenticated, service_role;

-- V1 remains installed for rollback/service compatibility, but browser roles can
-- no longer invoke the legacy SECURITY DEFINER path that predates currency and
-- public-location hardening.
revoke execute on function public.public_listing_search_page(
  text, text, text, text, numeric, numeric, integer, text, text, text,
  text, text, text, uuid, integer
) from anon, authenticated;

revoke execute on function private.public_listing_search_page(
  text, text, text, text, numeric, numeric, integer, text, text, text,
  text, text, text, uuid, integer
) from anon, authenticated;

comment on function public.public_listing_search_page_v2(
  text, text, text, text, text, text, numeric, numeric, integer, text, text,
  text, text, text, text, uuid, integer
) is 'Bounded public marketplace search using seller-native currency, privacy-safe location projection and keyset pagination.';
