...
as $function$
  ...
  where t.owner_id = auth.uid()
    ...
    and not exists (
      select 1 from public.peek_request_responses pr
      where pr.tour_id = t.id and pr.is_current
    )
...
begin
  ...
  return query
  select r.id, r.category, r.body, r.supporter_count, r.created_at
  ...
  where r.listing_id is not distinct from v_tour.listing_id
    ...
    and r.moderation_status = 'approved'
    and r.merged_into_id is null
  order by (r.status = 'pending') desc, r.supporter_count desc, r.created_at asc, r.id asc
  limit 100;
end;
$function$;
...
begin
  ...
  foreach v_request_id in array p_request_ids loop
    ...
    if v_request.status not in ('pending', 'answered', 'declined', 'expired')
       or v_request.moderation_status <> 'approved'
  ...
  v_link := case
    when v_tour.listing_id is not null then '/listing/' || v_tour.listing_id::text || '?responsePeek=' || p_tour_id::text || '#peek-threads'
    else '/service/' || v_tour.service_id::text || '?responsePeek=' || p_tour_id::text || '#peek-threads'
  end;
  ...
  insert into public.app_alerts (
    user_id, title, message, type, link, is_read, listing_id,
    event_type, source_key, created_at
  )
  select
    d.user_id,
    case when d.is_requester then 'Your Peek Request has been answered'
         else 'A Peek you wanted is now available' end,
    'The seller added new visual evidence. Watch the Response Peek on the listing.',
    'info', v_link, false, v_tour.listing_id,
    'peek_response_available', p_tour_id::text, now()
  from deduped d
