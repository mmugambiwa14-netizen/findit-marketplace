...
--
-- Keep the 0104 country-constrained plan while projecting only public columns
-- from the lateral candidate query. This preserves security-invoker behavior
-- under the intentionally narrow anon/authenticated column grants.
...
comment on function public.resolve_marketplace_location(double precision, double precision) is
  'Consent-gated, least-privilege nearest-place lookup strictly inside a supported country boundary; coordinates are not persisted.';
