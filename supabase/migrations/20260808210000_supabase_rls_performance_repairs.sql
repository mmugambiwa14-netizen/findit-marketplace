-- Preserve the existing RLS predicates while initializing auth.uid() once per
-- statement instead of once per candidate row. This is a performance-only
-- repair: visibility and write authorization remain unchanged.
