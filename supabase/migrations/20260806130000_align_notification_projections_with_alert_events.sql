...
-- app_alerts now permits twelve trusted event types, and notification_unread_count
-- counts every one of them. The two list projections never kept pace:
--
--   notification_rows      restricted rows to the original five event types, so the
--                          badge could report notifications this list never returned
--                          and the user had no way to read them from that view.
--   notification_rows_page returns every event type, but its p_event_type argument
--                          validation still rejected the newer ones, so filtering a
--                          list down to a tour or peek event raised 22023.
--
-- Both projections are realigned with the app_alerts contract: the row filter
-- matches the counter exactly, and the argument validation accepts every event
-- type the table is allowed to store.
--
-- 0101 moved these implementations into private behind invoker wrappers, so the
-- bodies are replaced there and the public wrappers are left untouched.
...
begin
  if to_regprocedure('private.notification_rows(text, boolean, integer, integer)') is null
    ...
    raise exception
      '20260806130000 expected the notification projections to live in private';
  end if;
end;
$migration$;
